#!/usr/bin/env bash
# OpeyScaler - Gradle'siz APK derleme betigi.
...
KEYSTORE="${KEYSTORE:-$OUT/opeyscaler.keystore}"
APK_NAME="${APK_NAME:-OpeyScaler}"
KEY_ALIAS="${KEY_ALIAS:-opeyscaler}"
KEY_PASS="${KEY_PASS:-opeyscaler}"
...
if [ ! -f "$KEYSTORE" ]; then
    ...
    keytool -genkeypair -v -keystore "$KEYSTORE" -alias "$KEY_ALIAS" \
        ...
        -dname "CN=OpeyScaler, OU=OpeyScaler, O=OpeyScaler, L=, S=, C=TR" >/dev/null 2>&1
fi
