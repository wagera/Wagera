...
public final class MainActivity extends Activity implements UpscaleJob.Listener {
    ...
    private void refreshTexts() {
        ...
        if (hasSource) {
            ...
            if (model.isNeural()) {
                sb.append(String.format(Locale.US, "\nModel %dx buyutur, kalan fark Lanczos ile tamamlanir.",
                        model.scale));
                long cost = (long) (srcWidth * (long) srcHeight * model.costPerPixel);
                if (cost > 400L * 1000 * 1000) {
                    sb.append("\nUyari: bu fotograf bu model icin buyuk, islem uzun surebilir.");
                }
            }
    ...
    private void startJob() {
        ...
        int[] t = preset.targetSize(srcWidth, srcHeight);
        UpscaleJob job = new UpscaleJob(sourceUri, preset, t[0], t[1], model, jpeg, jpegQuality(),
    ...
    @Override public void onJobChanged(UpscaleJob job) {
        ...
        String engine = job.usedModel != null && job.usedModel.isNeural()
                ? job.usedModel.label + (job.usedGpu ? " (GPU)" : " (CPU)")
